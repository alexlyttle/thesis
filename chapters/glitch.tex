...
\end{align}
%
where we expanded the imaginary error function \(\mathrm{erfi}(z)\) around \(z=0\) and drop terms of order \(\mathcal{O}(z^3)\). Parameters \(A_\heII = 2 \sqrt{2\pi} \, \Gamma_\heII \Delta_\heII \nu_0\), and \(\phi_\heII \approx 2\epsilon\), where we assume \(\epsilon\) is constant over the glitch region.

\todo{I have made a mistake here in the asymptotic integral. Try changing integral to be over a quantity that stays close to zero (like (tau - tauII) nu) and you may find the expansion of the error function makes more sense.}

Notice how this differs slightly from \citet{Houdek.Gough2007}, who appear to set the error function term to one. \(\mathbb{D}\)

...
\begin{table}
    \centering
    \caption{Observations of mode frequency \(\nu_n\) at radial order \(n\) for model star S (left) and 16 Cyg A (right). \(N\) are the number of `observed' radial orders and the scale of the Gaussian noise added to each column is given by \(\sigma_\obs\) where appropriate. The values and their uncertainties for 16 Cyg A come from \citet{Lund.SilvaAguirre.ea2017}.}
...
We used the asteroseismic benchmark star 16 Cyg A as an example with which to compare glitch fitting methods. We adopted values for 16 radial mode frequencies observed by \citet{Lund.SilvaAguirre.ea2017}. The mode frequencies and their associated uncertainties are given in Table \ref{tab:glitch-obs}. The glitch has been previously studied for 16 Cyg A \citep[with its binary companion 16 Cyg B][]{Verma.Faria.ea2014}, making it a useful subject for comparison.

\subsection{Methods}

We only modelled radial oscillation modes (\(l = 0\)), but these methods may be extended to higher angular degree in the future.
...
To estimate the probability of our model parameters given observations of mode frequencies, we used Bayes' theorem. Hence, we defined the posterior probability density as, 
%
\begin{equation}
...
\begin{gather*}
    \ln\beta_\helium \sim \mathcal{N}\left( \ln\overline{\beta}_\helium, \, 4 s_{\ln\tau}^2 \right), \quad \ln\alpha_\helium \sim \mathcal{N}\left[ 1/2 \, \ln({\overline{\beta}_\helium}/{400\pi}), \, s_{\ln\tau}^2 \right],\\
...
\begin{table}
    \centering
    \caption{The mean and variance for the prior normal distributions on each parameter where values are not explicit in text. \todo{add 16 Cyg}}
    \label{tab:glitch-prior}
    \input{tables/glitch-test-prior.tex}
\end{table}

The shape and scale parameters for the prior distributions are summarised in Table \ref{tab:glitch-prior}. \todo{Put this at top?}
...
Both methods yielded values of \(\tau_\helium\) within 1-\(\sigma\) of each other: \(917_{-53}^{+50} \, \mathrm{s}\) for the \citetalias{Verma.Raodeo.ea2019} method and \(931_{-88}^{+59} \, \mathrm{s}\) for the GP method.

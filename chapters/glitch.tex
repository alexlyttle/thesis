...
Secondly, we found a smaller amplitude \(\delta\nu\) at low frequency with the GP method than with the \citetalias{Verma.Raodeo.ea2019} method. This was particularly visible in the best case and 16 Cyg A. We found that this was a result of the different smooth background models \todo{A plot of the background model may help here}. The GP background component turns back towards the asymptotic solution at low frequency whereas the \citetalias{Verma.Raodeo.ea2019} method moves away from it. 

In the best case and 16 Cyg A, the GP method found a higher \(\tau_\helium\) than the  \citetalias{Verma.Raodeo.ea2019} method. It is possible that the GP is better able to pick up the small glitch signal from the first ionisation of helium than the polynomial. As a result, the GP method may be better at finding the true properties of the second ionisation glitch. For example, the large lower uncertainty in \(\tau_\helium\) for 16 Cyg A from the GP method could indicate solutions for the first ionisation being picked up.

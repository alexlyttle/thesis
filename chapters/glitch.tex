...
In Figure \ref{fig:gamma-temp-density}, we plot \(\gamma\) as a function of pressure and temperature for model S \citep[cf.][]{Houdayer.Reese.ea2021}. The first panel illustrates the three ionisation regions of hydrogen and helium. The first adiabatic exponent depends primarily on the temperature-density profile of the star. However, in the second panel we see that an increase in helium abundance decreases \(\gamma\) in the helium ionisation regions \todo{More discussion here}.
...
\begin{equation}
    \nu_n \sim \mathcal{GP}\left[ \widetilde{\nu}_n, k_\theta(n, n') \right]
\end{equation}
%
where \(\widetilde{\nu}_n\) are solutions to,
%
\begin{equation}
    \nu_n = \Delta\nu(n + \epsilon) + g(\nu_n)
\end{equation}

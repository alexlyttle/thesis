...
\end{equation}
%
where \(m\) and \(k\) are the mean and kernel functions which depend on parameters \(\vect{\theta}_B\)\footnote{We use the convention that some random variable \(y\) drawn from a distribution \(q\) given parameters \(x\) may be written as \(y \sim q(x)\).}. We can control our prior over the function space through the parametrisation of the kernel function. The mean function describes where we expect \(\nu_n\) to be given some approximate physical reasoning. We define our mean function as,
%
\begin{equation}
    m(n) = \tilde{\nu}_n + \delta_\helium(\tilde{\nu}_n) + \delta_\bcz(\tilde{\nu}_n); \quad \tilde{\nu}_n = (n + \varepsilon) \nu_0, \label{eq:asy-glitch}
\end{equation}
%
where \(\tilde{\nu}_n\) is  the asymptotic approximation for the mode frequency at radial order \(n\).

The kernel function represents a covariance matrix over \(n\). The kernel function should be chosen subjectively to capture the correlation between modes of different \(n\). We used the squared exponential kernel,
%
\begin{equation}
    k(\vect{n}, \vect{n}') = \alpha_k \nu_0 \, \ee^{- {||\vect{n} - \vect{n}'||^2}/{2\lambda_k^2}}
\end{equation}
%
where \(\alpha_k\) is an amplitude scale factor and \(\lambda_k\) is the length scale (in units of radial order). It was convenient to scale the amplitude by \(\nu_0\) since \todo{}. This describes the covariance between modes at different \(n\). The closer the modes are to each other, the higher the covariance. Both kernel parameters control the flexibility of the GP. The amplitude describes how far the function is likely to be from the mean function and the length scale describes the scale of correlation between different modes. As \(\lambda_k \rightarrow 0\), the off-diagonal terms of the kernel approach zero producing Gaussian noise with a variance of \(\alpha_k\nu_0\). We found values of \(\alpha_k = 0.5\) and \(\lambda_k = 5\) provided a suitable prior over function space, although these parameters may be further optimised to model stars in future work.

The GP likelihood for some set of observations \(\vect{\nu}_\obs = \{\nu_{n_i}^\obs\}_{i=1}^N\) at \(\vect{n}\) is a multivariate normal distribution with additional Gaussian noise terms added to the diagonal of the covariance matrix,
%
\begin{equation}
...
To estimate the probability of our model parameters given observations of mode frequencies, we used Bayes' theorem. We defined the posterior probability density as, 
%
\begin{equation}
...
\begin{gather*}
    \ln\tau_\helium \sim \mathcal{N}\left[ - \ln(10\overline{\nu}_0), \, s_{\ln\tau}^2 \right], \quad \ln\tau_\bcz \sim \mathcal{N}\left[- \ln(10\overline{\nu}_0/3), \, s_{\ln\tau}^2 \right],\\
    s_{\ln\tau}^2 = (1/5)^2 + (s_{\nu_0}/\overline{\nu}_0)^2,
\end{gather*}

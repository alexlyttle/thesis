...
\end{equation}
%
\todo{Explain the meaning of each term}. Differentiating both sides of Equation \ref{eq:var-prin} with respect to \(\omega\), we get,
%
\begin{align}
...
\end{align}
%
We may rewrite this as a function of the so-called structural kernels \(\mathcal{K}_{a,b}\),
%
\begin{equation}
...
\end{equation}
%
where \(\mathcal{K}_{a, b}\) gives the relative effect on \(\omega\) due to a perturbation in quantity \(a\) at fixed \(b\) at a given stellar radius. These kernels are defined in \citet{Gough.Thompson1991}. 

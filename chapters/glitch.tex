...
We fit Equation \ref{eq:1d-domega-func} to \(\delta\omega_n\) obtained from a glitch located at \(x_0 = 0.9\), with half-width \(\delta x = 0.02\), and change in sound speed \(\delta c = 0.03\). The best fitting line is shown in Figure \ref{fig:1d-fit}. Repeating this for different glitch parameters, it can be shown empirically how the `frequencies' \(\tau_1\) and \(\tau_2\) relate to the half-width and location of the glitch respectively. It emerges that \(\tau_1 \simeq 2\delta\tau\), where \(\delta\tau\) is half the acoustic width of the glitch (i.e. the time at which sound takes to transverse the region). The second `frequency' \(\tau_2 \simeq 2\tau_0\), where \(\tau_0\) is the acoustic depth from the nearest edge to the centre of the glitch, \(x_0\),
%
\begin{equation}
    \tau_0 = \int_0^{x_0'} \frac{\dd x}{c(x)},
\end{equation}
%
where \(x_0' = L/2 - | L/2 - x_0 |\) is the relative location of the glitch from the nearest edge of the system. Since the speed of sound \(c(x) \approx 1\) throughout the medium, \(x_0 \simeq L - \tau_0\). Our example fit yields \(\tau_1 \approx \SI{0.0389}{\second\per\radian}\) and \(\tau_2 \approx \SI{0.199}{\second\per\radian}\), approximately recovering the input glitch half-width of \num{0.02} and location of \num{0.9}.
...
%
\begin{equation}
    \tau(r_\star) = \int_R^{r_\star} \frac{\dd r}{c(r)} \equiv T_0 - \int_0^{r_\star} \frac{\dd r}{c(r)},
\end{equation}
%
where the acoustic radius of the star, \(\Tau = \tau(R)\) relates approximately to the average large frequency spacing \(\langle\Delta\nu_{nl}\rangle^{-1} \simeq 2T_0\).
...
In this section, we will first show how the sound speed inside a star is affected by the abundance of helium. Then, starting with the variational principle, we will derive a commonly used form of the glitch signature found in the mode frequencies of solar-like oscillators.
...
In Figure \ref{fig:gamma-temp-density} \todo{Add lines for different stellar mass and age to show degeneracy} we plot \(\gamma\) as a function of pressure and temperature for model S \citep[cf.][]{Houdayer.Reese.ea2021}. The first panel illustrates the three ionisation regions of hydrogen and helium. The first adiabatic exponent depends primarily on the temperature-density profile of the star. However, in the second panel we see that an increase in helium abundance decreases \(\gamma\) in the helium ionisation regions \todo{Show that increasing Y increases the depth and width of the glitch, but so do other parameters}.
...
To show the effect of small changes in \(\gamma\) on the sound speed, we plot the sound speed gradient in Figure \ref{fig:gamma-sound-speed}. The three models shown were evolved to the same central temperature with different initial helium abundances. We see how larger helium abundance increases the width and depression in \(\gamma\), which is reflected in the sound speed gradient. However, larger \(Y\) also leads to a relative reduction in hydrogen abundance. This slightly shrinks the width of the hydrogen ionisation region \todo{More discussion here}.
...
%
\begin{align}
    \mathcal{I} &\simeq \frac12 \omega \psi_0^2 \int_0^{T_0} [1 + \cos 2 (\omega\tau + \epsilon)] \, \dd \tau \simeq \frac12 \omega \psi_0^2 T_0.
\end{align}
%
...
%
\begin{equation}
    \left.\frac{\delta\omega}{\omega}\right|_\mathrm{osc} \simeq - \frac{1}{2T_0} \int \frac{\delta\gamma}{\gamma} \cos 2 (\omega\tau + \epsilon) \, \dd \tau,
\end{equation}
%
...
%
\begin{equation}
    \left.\frac{\delta\omega}{\omega}\right|_{\heII, \mathrm{osc}} \simeq \frac{\Gamma_\heII}{2\sqrt{2\pi} \, T_0} \, \int_{-\infty}^\infty \ee^{- x^2/2} \cos 2 (\Delta_\heII \omega x + \widetilde{\epsilon_\heII}) \, \dd x
\end{equation}
%
...
%
\begin{equation}
    \left.\frac{\delta\omega}{\omega}\right|_{\heII, \mathrm{osc}} \simeq \frac{\Gamma_\heII}{2 T_0} \ee^{- 2 \Delta_\heII^2 \omega^2} \cos 2 (\omega \tau_\heII + \epsilon_\heII),
\end{equation}
%
...
\end{equation}
%
where \(\phi_\heII = 2(\epsilon_\heII + \pi/4)\), and \(\nu_0 = (2 T_0)^{-1}\) is the inverse acoustic diameter of the star.
...
We will start by only modelling radial oscillation modes (\(l = 0\)), but this method may be extended to higher angular degree in the future. One way of doing this could be to introduce a new variable \(x = n + l/2\), or to expand the input dimensions.

\begin{equation}
    g(\vect{n}) \sim \mathcal{GP}\left[ m_\theta(\vect{n}), k_\theta(\vect{n}, \vect{n}') \right]
\end{equation}
%
where \(m\) and \(k\) are the mean and kernel functions. The mean function describes where we expect \(\nu_n\) to be. This should come from known physical processes. However, we do not know the full functional form of \(\nu_n\), so we introduce the kernel function to describe additional correlation between modes. The GP is effectively a prior over the function \(\nu_n = f(n)\). \todo{This is clunky, rethink}.

We know from the asymptotic theory of stellar oscillations, that the radial p-mode frequencies may be approximated by \citep[e.g.][]{Tassoul1980},
%
\begin{equation}
    \nu_n \simeq (n + \varepsilon) \nu_0
\end{equation}
%
where \(\nu_0\) is often approximated by the average large frequency separation \(\langle\Delta\nu\rangle\), and \(\varepsilon\) is a small phase offset which depends on the boundary conditions of the convective zone. This would not be a bad choice for mean function, although we know that there is more physics which contribute to the mode frequencies.

\begin{align}
    \nu_n &\simeq (n + \varepsilon)\nu_0 + g_\helium(\nu_n) + g_\bcz(\nu_n) \label{eq:asy-glitch}\\
    g_\helium(\nu) &= \alpha_\helium \nu_0 \nu \, \ee^{-\beta_\helium^2\nu^2} \sin(4\pi\tau_\helium\nu + \phi_\helium),\\
    g_\bcz(\nu) &= \alpha_\bcz^2 \nu_0 \nu^{-2} \, \sin(4\pi\tau_\bcz\nu + \phi_\bcz)
\end{align}
%
where \(g_\helium\) and \(g_\bcz\) are functions representing the acoustic glitch signature of helium ionisation and the base of the convective zone respectively. The parameters \(\alpha_\helium \simeq \Gamma_\heII\) and \(\beta_\helium \propto \Delta_\heII\) relate to the area and width of the depression in \(\gamma\) caused by the second ionisation of helium, and both have units of time. We neglect contribution by the first ionisation of helium to \(g_\helium(\nu)\) \todo{why?}. The amplitude parameter for the BCZ glitch, \(\alpha_\bcz \propto \sqrt{\Delta_{\bcz}}\) is relates to the difference in the second density derivative at the base of the convective zone and has units of frequency. The approximate acoustic depths of second helium ionisation and the BCZ are given by \(\tau_\helium\) and \(\tau_\bcz\) respectively, and \(\phi\) represents a phase constant.
...
%
\begin{align}
    f_n &\simeq n + \varepsilon + h_\helium(f_n) + h_\bcz(f_n)\\
    h_\helium(f) &= a_\helium f \, \ee^{-b_\helium^2 f^2} \sin(2 \pi \, t_\helium f + \phi_\helium),\\
    h_\bcz(f) &= (a_\bcz / f)^{2} \, \sin(2\pi\,t_\bcz f + \phi_\bcz)
\end{align}

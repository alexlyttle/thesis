...
From the variational principle \citep{Chandrasekhar1939,Chandrasekhar1964}, we can write the characteristic frequencies,
%
\begin{equation}
    \omega^2 = \frac{\mathcal{E}}{\mathcal{I}}\label{eq:var-prin}
\end{equation}
%
in terms of the mode interia,
%
\begin{equation}
    \mathcal{I} = \int_0^R \vec{\xi} \cdot \vec{\xi} \rho r^2 \dd r
\end{equation}
%
and its energy,
%
\begin{equation}
    \mathcal{E} = \int_0^R \left[\gamma P (\vec{\nabla}\cdot\vec{\xi})^2 + 2(\vec{\xi}\cdot\vec{\nabla} P) \vec{\nabla}\cdot\vec{\xi} + (\vec{\xi}\cdot\vec{\nabla} P) (\vec{\xi}\cdot\vec{\nabla}\ln\rho)\right] r^2 \dd r
\end{equation}
%
where \(\vec{\xi}\) is the Lagrangian perturbation of the pulsation mode. \todo{Explain the meaning of each term}. A structural glitch in the star would cause a small change in frequency (\(\delta\omega\)). Differentiating both sides of Equation \ref{eq:var-prin} with respect to \(\omega\), we get,
%
\begin{align}
...
\end{align}
%
To see the effect of a change in structure on the modes, it is possible to rewrite this as a function of the so-called structural kernels \(\mathcal{K}_{a,b}\),
%
\begin{equation}

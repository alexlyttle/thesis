...
In Figure \ref{fig:sound-speed-gradient}, we show the sound speed gradient of model S. We see how the speed of sound changes smoothly throughout the star, except for a wiggle around \SI{700}{\second}, a discontinuity at the base of the convective zone, and a rapid change near the core of the star. The first is caused by the ionisation of helium, which we explore further in Section \ref{sec:helium-glitch}. The second is due to a change in the density gradient as the structure moves from convectively unstable to radiative. \todo{Put a few references here of work which has studied these before}.

\begin{figure}
    \centering
    \includegraphics{example-image-a}
    \caption{The sound speed gradient, \(\dd c/\dd \tau\), of model S with respect to the acoustic depth, \(\tau\). The fractional radius, \(r/R\) is given on the top axis. The base of the convective zone is marked with a dashed line (or the convective zone is shaded).}
...
\begin{align}
    \begin{split}
        \eta(T, \rho) = \frac{1}{\partial_{TT}^2 f} &\left[n_\hydrogen y_\hydrogen \, (1 - y_\hydrogen) \, \frac{(\chi_\hydrogen / k_B T)^2}{2 - y_\hydrogen}\right.\\
        &\left. + \, n_\helium y_\helium^{(1)} \left(1 - y_\helium^{(1)}\right) \left(\frac{\chi_\helium^{(1)}}{k_B T}\right)^2 \right.\\
        &\left. + \, n_\helium y_\helium^{(2)} \left(1 - y_\helium^{(2)}\right) \left(\frac{\chi_\helium^{(2)}}{k_B T}\right)^2\right],
    \end{split}
\end{align}
%
...
\begin{align}
    \begin{split}
        \partial_{TT}^2 f = \frac{3}{2} &+ n_\hydrogen y_\hydrogen \left[ \frac{3}{2} + \frac{(1 - y_\hydrogen)}{2 - y_\hydrogen} \left(\frac{3}{2} + \frac{\chi_\hydrogen}{k_B T}\right)^2 \right]\\

...
The model parameters are \(\theta_A = (b_0, \dots, b_4, a_\helium, \beta_\helium, \tau_\helium, \phi_\helium, a_\bcz, \tau_\bcz, \phi_\bcz)\) where the glitch amplitude parameters \(a_i \equiv \alpha_i\nu_0\). These are optimised by minimising a \(\chi^2\) cost function with a regularisation term, 
%
\begin{equation}
    \chi^2 = \sum_n \left( \frac{\nu_n - f_{A}(n)}{\sigma_n} \right)^2 + \lambda^2 \sum_n \left( \frac{\dd^3 \tilde{\nu}_n}{\dd n^3} \right)^2.
\end{equation}
%
where the regularisation parameter is \(\lambda\).

Initial parameters are chosen uniformly at random within the bounds specified in \citet{Verma.Raodeo.ea2019}.

\subsubsection{The GP Method (B)}

In the second method we took a Bayesian approach,
%
\begin{equation}
    p(\theta_B \mid \nu_n) = \frac{p(\nu_n \mid \theta_B)\,p(\theta_B)}{p(\nu_n)} \equiv \frac{\mathcal{L}(\theta_B)\,p(\theta_B)}{\mathcal{Z}},
\end{equation}
%
where \(\mathcal{L}(\theta_B)\) is the likelihood of the data given the model, \(p(\theta_B)\) is the prior probability density of the model parameters, and \(\mathcal{Z}\) is the `evidence' (or normalisation).

We used a Gaussian Process (GP) instead of a high-order polynomial to model the correlated noise in the mode frequencies. The GP represents a probability distribution over function space, meaning that we can quantify the systematic uncertainty associated with the functional form of $f$ conditioned on the data. We write our model for a set of modes \(\vect{n} = \{n_i\}_N\) as a random draw from a GP,
%
\begin{equation}
    f_{B}(\vect{n}) \sim \mathcal{GP}\left[ m(\vect{n}), k(\vect{n}, \vect{n}') \right]
\end{equation}
%
where \(m\) and \(k\) are the mean and kernel functions which depend on parameters \(\theta_B\). We can control our prior over the function space through the parametrisation of the kernel function. The mean function describes where we expect \(\nu_n\) to be given some approximate physical reasoning. We define our mean function as,
%
\begin{equation}
    m(n) = \tilde{\nu}_n + \delta_\helium(\tilde{\nu}_n) + \delta_\bcz(\tilde{\nu}_n); \quad \tilde{\nu}_n = (n + \varepsilon) \nu_0, \label{eq:asy-glitch}
\end{equation}
%
where \(\tilde{\nu}_i\) is  the asymptotic approximation for the mode frequency at radial order \(n_i\).

The kernel function represents a covariance matrix over \(n\). We used the squared exponential kernel,
%
\begin{equation}
    k(\vect{n}, \vect{n}') = \alpha_k \nu_0 \, \ee^{- {||\vect{n} - \vect{n}'||^2}/{2\lambda_k^2}}
\end{equation}
%
where \(\alpha_k\) is the amplitude factor and \(\lambda_k\) is the length scale (in units of radial order). It was convenient to scale the amplitude by \(\nu_0\) since \todo{}. This describes the covariance between modes at different \(n\). The closer the modes are to each other, the higher the covariance. Both kernel parameters control the flexibility of the GP. The amplitude describes how far the function is likely to be from the mean function. The length scale describes the scale of correlation between different modes. As \(\lambda_k \rightarrow 0\), the off-diagonal terms of the kernel approach zero producing Gaussian noise with a variance of \(\alpha_k\nu_0\). We found values of \(\alpha_k = 0.5\) and \(\lambda_k = 5\) provided a suitable prior over function space, although these parameters may be further optimised to model stars in future work.

The GP likelihood for some set of observations \(\vect{\nu}_\obs\) at \(\vect{n}\) is a multivariate normal distribution with additional Gaussian noise terms added to the diagonal of the covariance matrix,
%
\begin{equation}
    \vect{\nu}_\obs \sim \mathcal{N}\left[ \vect{\mu}, \, \vect{K} + (\sigma^2 + \sigma_\obs^2) \vect{I} \right]
\end{equation}
%
where \(\vect{\mu} = m(\vect{n})\) and \(\vect{K} = k(\vect{n}, \vect{n})\). The scales of Gaussian noise in the model and observations are \(\sigma\) and \(\sigma_\obs\) respectively, and \(\vect{I}\) is an \(N \times N\) identity matrix. 

To make predictions with this model we draw from the following multivariate normal distribution \needcite[Rasmussen and Williams],
%
\begin{equation}
...
\todo{Prior should be a table/gathered equations with each parameter and their distribution}

Priors on \(\nu_0\) and \(\varepsilon\). In practice, the prior for \(\nu_0\) and \(\epsilon\) can come from global estimates of \(\langle\Delta\nu_n\rangle\) or a linear fit to the modes.
%
\begin{gather*}
...
The prior on the phase parameters \(\phi_\helium\) and \(\phi_\bcz\) was uniform from 0 to \(2\pi\). \todo{For \(\phi\) we map from unconstrained space to periodic space with modulo to allow for boundaries. Put this in a bit that explains the prior transform.}

Priors for the remaining parameters follow a log-normal distribution to ensure they are positive. We also exploit the property that the scale of a normal distribution in natural log-space is approximately the scale in real-space as a fraction of the distribution mean.

We based our prior on \(\tau_\helium\) and \(\tau_\bcz\) by observing how they scale with acoustic radius \(\tau_0\) in the grid of stellar models from \citep{Lyttle.Davies.ea2021}. Typically, \(\tau_\helium/\tau_0 \approx 0.2\) and \(\tau_\bcz/\tau_0 \approx 0.6\) for main sequence solar-like oscillators (see, e.g. Figure \ref{fig:gamma-sound-speed}). Therefore, we form our prior distributions from these relations using our prior on \(\nu_0\), with an additional spread of 20 per cent to account for variance with stellar properties.
...
\begin{gather*}
    ...
    \overline{\beta}_\helium = 32 \pi^2 \overline{\tau}_\helium^2 / 625,
\end{gather*}
...
\begin{equation*}
    \ln\alpha_\bcz \sim \mathcal{N}\left[ \ln 30\overline{\nu}_0, \, (4/5)^2 + (s_{\nu_0}/\overline{\nu}_0)^2\right], \quad \ln\sigma \sim \mathrm{N}\left( - \ln 100, 4\right),
\end{equation*}

The shape and scale parameters for the prior distributions are summarised in Table \ref{}.

\begin{table}
    \centering
    \caption{The mean and variance for the prior normal distributions on each parameter. \todo{add 16 Cyg}}
...
Using the glitch parameters as a helium diagnostic could involve measuring the amplitude of the helium glitch at a reference frequency such as \(\nu_{\max}\). We should expect solutions for this reference amplitude to converge on a value as the data quality increases from worst to best. It is also important that the uncertainty of the reference amplitude is accurate if using it to constrain helium abundance in a population of stars. We compared reference amplitudes at \(\nu_\mathrm{ref}\) from both methods in Figure \ref{fig:glitch-test-amplitude}. The \citetalias{Verma.Raodeo.ea2019} method preferred zero-amplitude solutions for the worst and better cases than the best case, which could bias population inference on towards lower values of helium abundance. On the other hand, the GP method reflects the prior in the worst case, with the width of the distribution shrinking as the data improves. In the best case, the \citetalias{Verma.Raodeo.ea2019} method found \(A_\mathrm{ref} = 0.347_{-0.005}^{+0.006}\), whereas the GP method found \(A_\mathrm{ref} = 0.297_{-0.043}^{+0.044}\).
...
\subsubsection{Correlation with helium}

\todo{Fit the three model stars and plot amplitude against helium abundance at surface and initial}.

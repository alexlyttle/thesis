...
\end{equation}
%
where \(\tilde{\epsilon} = \omega\tau_\heII + \epsilon_\heII\) and \(\epsilon=\epsilon_\heII\) is assumed constant accross the glitch region. We can solve the above integral analytically using differentiation under the integral sign by introducing an arbitrary variable \(a\),
%
\begin{equation}
...
\begin{align}
    ...
    &= 2\omega\Delta_\heII^2 \left[ \left. e^{- {t^2}/{2\Delta_\heII^2} } \sin 2 (a \omega t + \tilde{\epsilon}) \right|_{-\infty}^\infty - 2a\omega \int_{-\infty}^\infty e^{- {t^2}/{2 \Delta_\heII^2} } \cos 2 (\omega t + \tilde{\epsilon}) \, \dd t \right] \notag\\
    &= - 4 a \omega^2 \Delta_\heII^2 I(\omega)
\end{align}
%
...
\end{align}
%
Therefore, if we set \(a = 1\) to match Equation \ref{} we get the following equation for the oscillatory glitch signature \citep[cf.][]{Houdek.Gough2007},
%
\begin{equation}
